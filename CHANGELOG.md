...
- **Concurrent first fires no longer append the bar segment more than once.** Every pane's first hook fires at the same moment on a fresh tmux server, and the presence check read a `show-options -g` dump one IPC before the append — so several sessions each passed the check and each appended a copy, permanently, since every later fire then saw the token and skipped. The append now runs under a socket-keyed lock and re-reads `status-right` inside it. Reproduced on tmux 3.7b with three parallel installs (three segments before, one after)
- **`/clux:validate` now finds the plugin at all on a real marketplace install.** Every one of its four plugin lookups (`hooks.json`, `helpers.sh` twice, `path.sh`) assumed `.../clux/scripts/…`, but the marketplace cache nests a version directory in between (`.../clux/clux/<version>/scripts/…`), so all four matched nothing. Step 1 reported `FAIL plugin hooks.json not found` and every downstream check that used it was wrong; the self-install checks reported every healthy install as a stale marker and downgraded the one actionable Tier B warning to an INFO. All four patterns now allow that directory and pick the highest version when more than one cached copy is present
