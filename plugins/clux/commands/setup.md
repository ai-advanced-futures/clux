...
- **Bare machine** — after this command, the user has a complete working environment: sessions and windows visible in the bar, session navigation working, agent state showing.
- **Existing tmux setup** — after this command, every clux key and every clux visualization works, and **nothing else in that setup has changed**.
...
- **Deploy from the manifest** — `plugins/clux/config/deploy-manifest.txt`. Never write a script list into this file or into a shell block
